        ...
        <x-filament::section>
            ...
            <ul class="divide-y divide-slate-100 dark:divide-white/5">
                @forelse ($this->getRenewals() as $alert)
                    <li class="flex flex-col gap-2 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                        <div class="min-w-0 flex-1">
                            <div class="text-sm font-semibold leading-snug text-slate-900 dark:text-white">{{ $alert['title'] }}</div>
                            <div class="mt-0.5 text-xs text-slate-500">{{ $alert['site'] }}</div>
                        </div>
                        <span @class([
                ...
                @forelse ($this->getMissingLogs() as $site)
                    <li class="flex flex-col gap-2 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                        <div class="flex min-w-0 items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <div class="text-sm font-semibold leading-snug text-slate-900 dark:text-white">{{ $site['name'] }}</div>
                                <div class="text-xs text-slate-500">{{ $site['city'] }}</div>
                            </div>
                        </div>
                        <span class="inline-flex w-fit shrink-0 rounded-full bg-warning-50 px-2.5 py-1 text-xs font-semibold text-warning-700 dark:bg-warning-400/10 dark:text-warning-400">
                            {{ __('No log today') }}
                        </span>
                    </li>
                @empty
